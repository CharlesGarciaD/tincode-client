...
export function UserFormModal({ user, onClose, onSaved }) {
  ...
  const [avatarPreview, setAvatarPreview] = useState(
    user?.avatar ? `${AVATAR_BASE_URL}/${user.avatar}` : null
  );
      ...
      <div className="user-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="user-modal__form">
          <div className="user-modal__avatar-section">
            <label htmlFor="avatar-input" className="user-modal__avatar-preview">
              {avatarPreview ? (
                <img src={avatarPreview} alt="Avatar" />
              ) : (
                <span>{formValues.firstname.charAt(0).toUpperCase() || "U"}</span>
              )}
              <div className="user-modal__avatar-overlay">Cambiar</div>
            </label>
          ...
          <div className="user-modal__row">
            ...
            {!isEdit && (
              <div className="user-modal__field user-modal__field--checkbox">
                <label>
                  <input
                    type="checkbox"
                    name="active"
          ...
          {serverError && <p className="user-modal__error">{serverError}</p>}
