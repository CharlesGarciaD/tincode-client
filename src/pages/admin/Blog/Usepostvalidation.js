...
// Slug simple: minúsculas, números, guiones (sin espacios ni caracteres especiales)
...
export function usePostValidation() {
  ...
  const validate = useCallback((form, { isEdit = false } = {}) => {
    ...
    // Contenido (viene de TinyMCE, puede incluir HTML vacío tipo "<p></p>")
    ...
    if (!form.path || form.path.trim() === "") {
      ...
      newErrors.path =
        "Usa solo minúsculas, números y guiones (ej: mi-nuevo-post).";
    }
