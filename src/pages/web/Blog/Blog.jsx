...
export function Blog() {
  ...
  return (
    <div className="tc-blog-page">
      <div className="tc-blog-page__header">
        <span className="tc-blog-page__eyebrow">// blog</span>
        <h1>Blog</h1>
        <p>
      ...
      <div className="tc-blog-page__inner">
        ...
        {!loading && !error && posts.length > 0 && (
          <div className="tc-blog-grid">
            {posts.map((post) => (
              <Link key={post._id} to={`/blog/${post._id}`} className="tc-post-item">
                <div className="tc-post-item__image">
                  {post.miniature && !brokenImages[post._id] ? (
                    <img
                      src={`${POST_IMAGE_BASE_URL}/${post.miniature}`}
                      alt={post.title}
                      onError={() =>
                        setBrokenImages((prev) => ({ ...prev, [post._id]: true }))
                      }
                    />
                  ) : (
                    <div className="tc-post-item__placeholder">📝</div>
                  )}
                </div>

                <div className="tc-post-item__body">
                  <span className="tc-post-item__date">
        ...
        {!loading && totalPages > 1 && (
          <div className="tc-blog-pagination">
            ...
            <div className="tc-blog-pagination__pages">
