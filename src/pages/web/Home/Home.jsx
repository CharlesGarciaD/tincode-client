...
export function Home() {
  ...
  const [brokenCourseImages, setBrokenCourseImages] = useState({});
  ...
  return (
    <div className="tc-home">
      ...
      <section className="tc-hero">
        <div className="tc-hero__inner">
          <div className="tc-hero__copy">
            <span className="tc-hero__eyebrow">$ whoami --learn-to-code</span>
            <h1 className="tc-hero__title">
              Aprende a programar.
              <br />
              <span className="tc-hero__title-accent">De verdad.</span>
            </h1>
            <p className="tc-hero__subtitle">
              Cursos prácticos, artículos escritos por desarrolladores reales
              y una comunidad que avanza al ritmo de la industria — no al de
              un curso grabado hace cinco años.
            </p>
            <div className="tc-hero__actions">
              <a href="/courses" className="tc-hero__cta-primary">
                Empieza gratis
              </a>
              <a href="/blog" className="tc-hero__cta-secondary">
                Leer el blog →
              </a>
            </div>
          </div>
          ...
          <div className="tc-hero__visual">
            <div className="tc-editor">
              <div className="tc-editor__bar">
                <span className="dot dot--red" />
                <span className="dot dot--yellow" />
                <span className="dot dot--green" />
                <span className="tc-editor__filename">tincode.js</span>
              </div>
              <div className="tc-editor__body">
                {CODE_SNIPPET.map((line, i) => (
                  <div
                    key={i}
                    className="tc-editor__line"
                    style={{ paddingLeft: `${line.indent * 20}px` }}
                  >
                    <span className="tc-editor__lineno">{i + 1}</span>
      ...
      <section className="tc-section">
        <div className="tc-section__inner">
          <div className="tc-section__header">
            <div>
              <span className="tc-section__eyebrow">// cursos</span>
              <h2>Cursos destacados</h2>
            </div>
            <a href="/courses" className="tc-section__link">
              Ver todos →
            </a>
          </div>

          {loadingCourses && <p className="tc-section__state">Cargando cursos...</p>}

          {!loadingCourses && courses.length === 0 && (
            <p className="tc-section__state">
          ...
          {!loadingCourses && courses.length > 0 && (
            <div className="tc-card-grid">
              {courses.map((course) => (
                ...
                >
                  <div className="tc-course-card__image">
                    {course.miniature && !brokenCourseImages[course._id] ? (
                      <img
                        src={`${COURSE_IMAGE_BASE_URL}/${course.miniature}`}
                        alt={course.title}
                        onError={() =>
                          setBrokenCourseImages((prev) => ({ ...prev, [course._id]: true }))
                        }
                      />
                    ) : (
                      <div className="tc-course-card__placeholder">🎓</div>
                    )}
                    {course.score !== undefined && course.score !== null && (
                      <span className="tc-course-card__score">
                        ⭐ {course.score}
                      </span>
                    )}
                  </div>
                  <div className="tc-course-card__body">
                    ...
                    <span className="tc-course-card__price">
                      S/ {course.price}
                    </span>
                  </div>
                </a>
      ...
      <section className="tc-section tc-section--muted">
        <div className="tc-section__inner">
          <div className="tc-section__header">
            <div>
              <span className="tc-section__eyebrow">// blog</span>
              <h2>Últimos artículos</h2>
            </div>
            <a href="/blog" className="tc-section__link">
              Ver todos →
            </a>
          </div>

          {loadingPosts && <p className="tc-section__state">Cargando artículos...</p>}
            ...
            <div className="tc-card-grid">
              {posts.map((post) => (
                <a key={post._id} href={`/blog/${post._id}`} className="tc-post-card">
                  <div className="tc-post-card__image">
                    {post.miniature && !brokenPostImages[post._id] ? (
                      <img
                        src={`${POST_IMAGE_BASE_URL}/${post.miniature}`}
                        alt={post.title}
                        onError={() =>
                          setBrokenPostImages((prev) => ({ ...prev, [post._id]: true }))
                        }
                      />
                    ) : (
                      <div className="tc-post-card__placeholder">📝</div>
                    )}
                  </div>
                  <div className="tc-post-card__body">
